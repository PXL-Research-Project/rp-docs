...
System_Boundary(s1, "Online Boutique Web Application") {
  ...
  Container(frontend, "Frontend (Web UI + Edge API)", "Go HTTP server", "Serveert UI, beheert sessie-id, orchestreert user flows; roept backend services aan via gRPC.")

  Container(checkout, "Checkout Service", "Go (gRPC)", "Orkestreert checkout: cart ophalen, payment, shipping, email bevestiging.")
  Container(cart, "Cart Service", ".NET/C# (gRPC)", "Leest/schrijft winkelwagen items in Redis.")
  Container(product, "Product Catalog Service", "Go (gRPC)", "Productlijst, zoeken, productdetails. (Brondata/DB).")
  Container(currency, "Currency Service", "Node.js (gRPC)", "Converteert bedragen tussen valuta; haalt real-world koersen op.")
  Container(payment, "Payment Service", "Node.js (gRPC)", "Verwerkt creditcard charge (mock) en geeft transactie-id terug.")
  Container(shipping, "Shipping Service", "Go (gRPC)", "Berekening shipping cost en verzendproces (mock).")
  Container(email, "Email Service", "Python (gRPC)", "Verstuurt order confirmation email (mock).")
  Container(reco, "Recommendation Service", "Python (gRPC)", "Aanbevelingen op basis van items in cart.")
  Container(ads, "Ad Service", "Java (gRPC)", "Tekst-ads op basis van contextwoorden.")
  ...
  Container(loadgen, "Load Generator", "Locust", "Simuleert realistische user flows (test/benchmark).")
}
