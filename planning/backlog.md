...
- [ ] **Private IP Only:** Instances in private subnets hebben **geen** public IP.
- [ ] **Security Baseline:** Security groups beperken verkeer tot minimum (ALB -> app, app -> DB).
- [ ] **Bewijs:** Screenshot of export van VPC, subnets en route tables.
...
- [ ] **Public Access:** RDS staat op `Publicly accessible = false`.
- [ ] **Backups:** Automatische backups zijn ingeschakeld.
- [ ] **Bewijs:** Connectie vanaf app-host werkt en publieke connectie faalt.
...
- [ ] **Service Start:** Services starten automatisch na reboot (systemd of PM2).
- [ ] **Bewijs:** `curl` of `health` endpoint toont een succesvolle response.
...
- [ ] **Bewijs:** `curl` op de root geeft `200` en de UI laadt.
...
- [ ] **Health Path:** Health check path en expected status code zijn ingesteld.
...
- [ ] **Base Playbook:** Een Ansible playbook dat op *alle* servers:
...
- [ ] **Bewijs:** Output van een tweede run toont `changed=0`.
...
- [ ] **Bewijs:** `systemctl status` toont `active (running)` na reboot.
...
- [ ] **Bewijs:** Rollback is getest en gedocumenteerd.
...
- [ ] **Toegang:** Wie de vault mag openen en waar het wachtwoord wordt bewaard is beschreven.
...
- [ ] **Validatie:** `dig`/`nslookup` resolve toont de juiste target.
...
- [ ] **HTTPS:** SSL-certificaat is actief (ACM).
- [ ] **Routing:** Frontend en API routes worden correct doorgestuurd.
- [ ] **Idempotency:** Playbook kan opnieuw draaien zonder onnodige wijzigingen.
- [ ] **Bewijs:** Listener rules en target groups zijn gedocumenteerd.
...
- [ ] **Claim Mapping:** Header/claim mapping is geconfigureerd en getest.
- [ ] **Bewijs:** Ongeauthenticeerde request krijgt 401, geauthenticeerde request slaagt.
...
- [ ] **Idempotency:** Playbook is herhaalbaar zonder drift.
...
- [ ] **Capacity Limits:** Min/Max/Desired capacity is ingesteld en gedocumenteerd.
- [ ] **Warmup/Cooldown:** Instance warmup en cooldown policy zijn vastgelegd.
- [ ] **Health Check Type:** EC2 of ELB health checks zijn expliciet gekozen en gedocumenteerd.
...
- [ ] **Alerting:** Minstens 1 alert rule is actief en getest.
- [ ] **Correlatie:** Er is een voorbeeld van correlatie tussen log en metric.
...
- [ ] Testprofiel (duur, requests/sec, dataset) is vastgelegd.
- [ ] Succescriteria (bv. error rate, latency target) zijn gedefinieerd.
